...
local CHLORPHOS_TRANSITION = 6 
local CHLORPHOS_INTENSITY = 1 
...
local function GetChlorPhosBlurFactor()
    ...
    if highStart == 0 then
        ...
        highStart = ply:GetNWFloat("npc_chlorphos_high_start", 0)
        highEnd = ply:GetNWFloat("npc_chlorphos_high_end", 0)
    end
...
hook.Add("HUDPaint", "NPCChlorPhosGasThrow_Overlay", function()
    ...
    local pulse = math.sin(t * 1.8) * 0.5 + 0.5 
    local washAlpha = math.floor((18 + pulse * 28) * factor) 
